...
## Script by Tobias Rausch

if [ $# -lt 4 ]
then
    echo ""
    echo "Usage: $0 <TSS bed> <peak.file.lst> <bam.file.lst> <output prefix> [PEAKS|FOOTPRINTS]"
    echo ""
    exit -1
fi
...
do
	...
	echo "Reading" ${P} ", Avg. peak width" `zcat ${P} | awk '{SUM+=($3-$2);} END {print SUM/NR;}'`
	zcat ${P} >> ${OP}.concat.peaks
    fi
done
...
bedtools intersect -wa -a <(zcat ${OP}.counts.gz | tail -n +2) -b <(cat ${ATYPE}) | sort -k1,1V -k2,2n | uniq >> ${OP}.tss.counts
zcat ${OP}.counts.gz | head -n 1 > ${OP}.notss.counts
bedtools intersect -v -wa -a <(zcat ${OP}.counts.gz | tail -n +2) -b <(cat ${ATYPE}) | sort -k1,1V -k2,2n | uniq >> ${OP}.notss.counts
